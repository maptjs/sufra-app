...
class _ScanScreenState extends State<ScanScreen> {
  ...
  bool get _soundEnabled => context.read<SettingsProvider>().soundEnabled;
  ...
  Future<void> _onDetect(BarcodeCapture capture) async {
    ...
    if (_soundEnabled) await _sound.playShutter();
    await _controller.stop();

    try {
      // Offline-first: if we've scanned this barcode before, show it
      // instantly with no network call — and it still works with no
      // internet connection at all.
      final cached = await _storage.getCachedProduct(barcode);
      final product = cached ?? await OpenFoodFactsService().lookup(barcode);
      if (!mounted) return;
      ...
      builder: (_) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(message, textAlign: TextAlign.center, style: TextStyle(color: SufraColors.text(context), fontSize: 15)),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'سُفرة تحتاج إلى إذن الكاميرا لمسح الباركود.\nيرجى تفعيل إذن الكاميرا من إعدادات الهاتف.',
        ...
        children: [
          ...
          SafeArea(
            child: Padding(
              ...
              child: Row(
                children: [
                  _circleButton(Icons.arrow_back_ios_new_rounded, () => Navigator.pop(context), 'رجوع'),
                  const Spacer(),
                  _circleButton(
                    _torchOn ? Icons.flash_on_rounded : Icons.flash_off_rounded,
                    _toggleTorch,
                    _torchOn ? 'إيقاف الفلاش' : 'تشغيل الفلاش',
                  ),
                  const SizedBox(width: 10),
                  _circleButton(Icons.keyboard_rounded, _manualEntry, 'إدخال الباركود يدويًا'),
                ],
              ),
            ),
          ),
  ...
  Widget _circleButton(IconData icon, VoidCallback onTap, String semanticLabel) {
    return Semantics(
      label: semanticLabel,
      button: true,
      child: InkWell(
